...
# def av_symbol_search(
#     query: str,
#     av_api_url: str | None = None,
#     av_api_key: str | None = None
#     ,*, region_preference=("United States", "United Kingdom", "Canada")) -> dict:
#     """
...
#     """
#     av_key = av_api_key or ALPHA_VINTAGE_API_KEY
#     av_url = av_api_url or ALPHA_VINTAGE_API_URL
#     if not av_key or not av_url:
...
# Fetch API keys and URLs from environment variables
...
def av_symbol_search(
    ...
    av_api_key: str | None = None,
    *,
    region_preference=("United States", "United Kingdom", "Canada"),
) -> dict:
    """
    Return the best match dict from Alpha Vantage SYMBOL_SEARCH.
    Strategy: highest matchScore, preferring preferred regions if present.
...
def av_time_series_daily(
    ticker: str, av_api_url: str | None = None, av_api_key: str | None = None
) -> dict:
    """
    Return monthly time series for ticker from Alpha Vantage TIME_SERIES_MONTHLY.
    Returns dict with keys: meta, time_series (date->data)
    ...
    params = {"function": "TIME_SERIES_MONTHLY", "symbol": ticker, "apikey": av_key}
    r = requests.get(av_url, params=params, timeout=15)
    r.raise_for_status()
    data = r.json()
    if "Error Message" in data:
        raise RuntimeError(
            f"Error from Alpha Vantage for '{ticker}': {data['Error Message']}"
        )
    meta = data.get("Meta Data", {})
    ts = data.get("Monthly Time Series", {}) or {}
    if not ts:
    ...
    for date_str, vals in ts.items():
        try:
            ...
            print(
                f"Skipping invalid date entry '{date_str}' for '{ticker}': {e}",
                file=sys.stderr,
            )
...
def get_av_daily_data(
    ticker, av_api_url: str | None = None, av_api_key: str | None = None
) -> dict:
    """
    Get the daily time series data for a given ticker from Alpha Vantage API.
    """
